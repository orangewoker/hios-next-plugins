# 图片对比

在无限画布中连接图片 A 与图片 B，直接在独立插件节点内拖动分割线比较两张图片，并可输出当前遮罩对比图。

## 0.3.4

- 分割线拖动改为逐帧更新画面、松手后再保存状态，避免连续状态写入导致拖动卡住。
- 补全指针捕获丢失与取消处理，拖动移出分割线后仍可稳定结束。

## 0.3.3

- 隐藏输入、输出端口旁的文字标签，但完整保留连接圆点、悬停提示、连线和工作流输入输出。
- 删除画面分割线中间的 `↔` 圆形图标，只保留清晰的垂直分割线。
- 配合 HIOS Next 通用插件节点尺寸持久化接口，重新进入画布后恢复用户调整的节点宽高。

## 0.3.2

- 修复已有图片仍被空状态遮罩覆盖、看起来发白或淡化的问题。
- 移除底部重复的滑块条，只保留画面中的分割线拖动交互。
- 执行按钮统一改为“运行”，运行结果直接显示在节点并可从图片端口输出。

## 0.3.1

- 输入端口改为清晰的图片 A / 图片 B 用户文案；宿主通用插件节点会显示明确的输入、输出端口标签。

## 0.3.0

- 已移除宿主 `builtin` 适配器，界面、逻辑和输出全部位于插件自己的 `runtime/index.html`。
- 两张图片固定尺寸完整叠放；滑杆只通过 `clip-path` 改变 A 图可见范围，不改变两图尺寸。
- 支持主题同步、滑杆状态持久化、画布 `run` 协议和 PNG 图片输出端口。
- 安装或更新后由 HIOS 动态刷新 renderer，无需为本插件再次修改或重打包主程序。

安装 URL：`https://github.com/orangewoker/hios-next-plugins/tree/main/plugins/image-compare`
